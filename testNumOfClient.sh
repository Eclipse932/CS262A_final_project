...
do
  ...
  do
    if [[ `expr $i % 2` -eq 0 ]]
      then
      echo $i
      java database/ClientApp  responder0 ../test-file/client$i/filename.txt > "../result/clientApp/$j/clientAppLog${i}-${j}.txt"  &
      else
      java database/ClientApp  responder1 ../test-file/client$i/filename.txt > "../result/clientApp/$j/clientAppLog${i}-${j}.txt"  &
    fi
  done
  read
done
